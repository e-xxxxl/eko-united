...
export default function NewMatchPage() {
  ...
  async function handleSave(values: MatchFormValues) {
    ...
    setError("");
    try {
      await adminFetch("/matches", { method: "POST", body: JSON.stringify(values) });
    ...
    <AdminShell>
      {() => (
        <div className="px-6 py-16 sm:px-10">
          ...
          <h1 className="display-title font-display mt-2 mb-10">Add Match</h1>
          <MatchForm onSave={handleSave} saving={saving} error={error} />
        </div>
      )}
    </AdminShell>
  );
